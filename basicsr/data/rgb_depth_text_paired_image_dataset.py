...
def _b3_root_list(value, option_name):
...
class RGBDepthTextPairedImageDataset(data.Dataset):
    """Stage-1 B3 dataset: paired LR RGB/depth, HR RGB, and caption.

    B3 change note: depth is normalized on the complete LR image before the
    RGB/depth/GT synchronized crop and augmentation. Text is paired strictly by
    GT basename and remains a global full-image description.
    """
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt.get('type') != 'disk':
            raise NotImplementedError('RGBDepthTextPairedImageDataset supports disk folders only.')

        self.mean = opt.get('mean')
        self.std = opt.get('std')
        self.filename_tmpl = opt.get('filename_tmpl', '{}')
        self.filename_tmpl_depth = opt.get('filename_tmpl_depth', self.filename_tmpl)
        ...
        self.depth_percentile_low = float(opt.get('depth_percentile_low', 0.02))
        self.depth_percentile_high = float(opt.get('depth_percentile_high', 0.98))
        self.depth_normalize_eps = float(opt.get('depth_normalize_eps', 1e-6))
        ...
        gt_roots = _b3_root_list(opt['dataroot_gt'], 'dataroot_gt')
        lq_roots = _b3_root_list(opt['dataroot_lq'], 'dataroot_lq')
        depth_roots = _b3_root_list(opt['dataroot_depth'], 'dataroot_depth')
        text_roots = _b3_root_list(opt['dataroot_text'], 'dataroot_text')
        if not (len(gt_roots) == len(lq_roots) == len(depth_roots) == len(text_roots)):
            raise ValueError('B3 GT, LR, depth, and text root lists must have equal lengths.')

        self.paths = []
        for gt_root, lq_root, depth_root, text_root in zip(
                gt_roots, lq_roots, depth_roots, text_roots):
            ...
            for gt_name in sorted(scandir(gt_root)):
                ...
                depth_name = f'{self.filename_tmpl_depth.format(basename)}{image_ext}'
                ...
                if depth_name not in depth_names:
                    raise FileNotFoundError(f'Missing paired depth image: {osp.join(depth_root, depth_name)}')
        ...
        if not self.paths:
            raise ValueError('RGBDepthTextPairedImageDataset found no paired samples.')
    ...
    def __getitem__(self, index):
        ...
        raw_depth = imfrombytes(
            self.file_client.get(paths['depth_path'], 'depth'), flag='unchanged', float32=False)
        ...
        if not text:
            raise ValueError(f'B3 requires a non-empty text caption: {paths["text_path"]}')

        if img_lq.shape[:2] != img_depth.shape[:2]:
            raise ValueError(
                'B3 requires aligned LR RGB and depth images, but got '
        ...
        if self.opt['phase'] == 'train':
            img_gt, (img_lq, img_depth) = paired_random_crop(
                img_gt, [img_lq, img_depth], self.opt['gt_size'], scale, paths['gt_path'])
            img_gt, img_lq, img_depth = augment(
                [img_gt, img_lq, img_depth], self.opt['use_hflip'], self.opt['use_rot'])
